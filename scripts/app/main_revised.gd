...
# Roll-to-Move Feel Pass timings. Keep these centralized so QA can tune the
# complete stop-to-move rhythm without hunting through await chains.
const ROLL_SETTLING_SECONDS := 0.14
const ROLL_RESULT_LOCK_SECONDS := 0.20
const ROLL_SLOT_TRANSFER_SECONDS := 0.14
const ROLL_MOVE_PREP_SECONDS := 0.10
...
var moving: bool = false:
	set(value):
		...
		if _syncing_phase_flags:
			return
		# Compatibility bridge for legacy callers. New code must set turn_phase
		# directly; this only prevents old `moving = false` cleanup paths from
		# leaving the UI stuck in a post-roll phase.
		if moving:
			if turn_phase == TurnPhase.READY:
				turn_phase = TurnPhase.MOVING
		elif turn_phase in [TurnPhase.SETTLING, TurnPhase.RESULT_LOCK, TurnPhase.SLOT_TRANSFER, TurnPhase.MOVE_PREP, TurnPhase.MOVING, TurnPhase.RESOLVING_TILE]:
...
func _wait_roll_sequence(duration: float, sequence_id: int) -> bool:
	if not _is_roll_sequence_current(sequence_id):
		return false
	await get_tree().create_timer(maxf(0.0, duration), false).timeout
	# Application pause or an in-game pause must not turn a valid sequence into
	# an abort merely because its timer completed while PAUSED. Wait for the
	# phase to be restored, while still honoring cancellation and scene exit.
	while _is_roll_sequence_current(sequence_id) and turn_phase == TurnPhase.PAUSED:
		await get_tree().process_frame
	return _is_roll_sequence_current(sequence_id)
...
func _on_roll_pressed() -> void:
	...
	if dice_values.is_empty():
		return
	if dice_mode == 5:
		# The roll presentation is complete, but movement must wait for the
		# player's three-die selection. RESULT_LOCK keeps roll input disabled
		# without pretending the board is already moving.
...
func _animate_dice_roll(count: int, extra_controls_parent: VBoxContainer = null) -> Array[int]:
	...
	# All gameplay and presentation after this point use the committed snapshot.
	# Preview values may keep changing internally in another presenter, but they
	# can no longer alter the face the player stopped or the value sent to rules.
	var final_values: Array[int] = committed_values.duplicate()
	for index: int in range(final_values.size()):
		if final_values[index] <= 0:
			final_values[index] = clampi(rolling_values[index], 1, 6)
	rolling_values = final_values.duplicate()

	# サイコロ停止演出 (SETTLING) 開始
	turn_phase = TurnPhase.SETTLING
	_render_dice(final_values, false)
	...
	# SETTLING時間 (140ms)
	if not await _wait_roll_sequence(ROLL_SETTLING_SECONDS, sequence_id):
		_abort_map_dice_roll()
		return []
	...
	if map_overlay_roll:
		if count == 3:
			var slot_roles: Dictionary = DiceLogicScript.evaluate_current(final_values, count)
			var slot_labels: Array = slot_roles.get("labels", [])
			map_dice_overlay.show_slot_result(" + ".join(slot_labels) if not slot_labels.is_empty() else "DICE SLOT", final_values, slot_labels)
			_play_flow_pulse(&"role_resolved")
		if count != 5:
			var preview_distance := maxi(0, _sum_dice_values(final_values) + GameState.next_move_bonus)
	...
	if not await _wait_roll_sequence(ROLL_RESULT_LOCK_SECONDS, sequence_id):
		_abort_map_dice_roll()
		return []
	
	# SLOT_TRANSFER (140ms)
	turn_phase = TurnPhase.SLOT_TRANSFER
	if not await _wait_roll_sequence(ROLL_SLOT_TRANSFER_SECONDS, sequence_id):
		_abort_map_dice_roll()
		return []
	...
	if not await _wait_roll_sequence(ROLL_MOVE_PREP_SECONDS, sequence_id):
		_abort_map_dice_roll()
		return []
	...
	turn_phase = TurnPhase.MOVING
	if map_overlay_roll:
		await map_dice_overlay.hold_and_return(final_values)
		if not _is_roll_sequence_current(sequence_id):
			_abort_map_dice_roll()
			return []
		if count != 5:
...
func _abort_map_dice_roll() -> void:
	...
	SaveManager.save_now()
	turn_phase = TurnPhase.READY
	phase_before_pause = TurnPhase.READY
	locked_dice_count = 0
	rolling_values.clear()
	committed_values.clear()
	if is_instance_valid(dice_audio):
	...
	stop_all_button.visible = false
	if is_instance_valid(active_extra_left_stop):
		active_extra_left_stop.disabled = true
	if is_instance_valid(active_extra_all_stop):
		active_extra_all_stop.disabled = true
	active_extra_left_stop = null
	active_extra_all_stop = null

func _lock_next_die(automatic: bool) -> void:
	if not can_stop_roll() or locked_dice_count >= rolling_values.size():
		return
...
func _stop_all_dice() -> void:
	if not can_stop_roll() or locked_dice_count >= rolling_values.size(): return
...
func _resolve_roll(values: Array[int]) -> void:
	# Rule evaluation and transaction preparation happen after the presentation
	# handoff, but the board is still in its movement phase until the path has
	# actually completed. Landing effects switch to RESOLVING_TILE later.
	turn_phase = TurnPhase.MOVING
...
func _continue_roll_transaction() -> void:
	...
	if phase == "RESULT_COMMITTED":
		...
		SaveManager.save_now()
	elif phase in ["MOVEMENT_COMMITTED", "SPACE_EFFECT_COMMITTED"]:
		turn_phase = TurnPhase.RESOLVING_TILE
	...
	if turn_phase != TurnPhase.RESOLVING_TILE:
		turn_phase = TurnPhase.RESOLVING_TILE
...
func _qa_tourmap() -> void:
	...
	var wrap_ok: bool = wrapped_indices == [88, 89, 0, 1, 2, 3, 4, 5]
