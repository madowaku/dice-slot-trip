...
func _init() -> void:
	...
	var transaction_id := str(state.roll_transaction.roll_transaction_id)
	_expect(str(state.roll_transaction.phase) == "PRE_ROLL" and transaction_id != "", "PRE_ROLL reservation")
