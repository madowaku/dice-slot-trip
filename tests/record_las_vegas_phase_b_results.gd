...
func _record() -> void:
	output_dir = OS.get_environment("LAS_VEGAS_PHASE_B_OUTPUT_DIR")
	if output_dir.is_empty():
		push_error("LAS_VEGAS_PHASE_B_OUTPUT_DIR is required")
		quit(2)
		return
	...
	print("LAS_VEGAS_PHASE_B_RESULTS size=%s failures=%d output=%s actual_rendering=%s" % [OUTPUT_SIZE, failures, output_dir, true])
