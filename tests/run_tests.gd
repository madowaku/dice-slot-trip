...
func _init() -> void:
	...
	_expect(DiceLogicScript.evaluate([1, 2, 3]).main == &"STRAIGHT" and DiceLogicScript.evaluate([3, 2, 1]).main == &"STRAIGHT", "STRAIGHT accepts ordered ascending and descending runs")
	_expect(DiceLogicScript.evaluate([1, 3, 2]).main == &"", "STRAIGHT rejects a shuffled consecutive set")
