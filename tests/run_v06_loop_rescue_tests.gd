...
func _test_session_rescue_and_save() -> void:
	...
	_expect(session.position() == {"route_id":"loop_oasis_ring", "tile_index":3} and session.acknowledge_resolution(), "fixture enters W1 and clears its first slot set")
	_roll_and_finish(session, 6)
	_expect(session.position().tile_index == 1 and session.loop_wrap_count() == 1, "first EXIT pass remains in the ring and records one wrap")
	_roll_and_finish(session, 6)
	_roll_and_finish(session, 2)
	_expect(session.position().tile_index == 1 and session.loop_wrap_count() == 2 and session.acknowledge_resolution(), "second EXIT pass remains playable and records two wraps")
	...
	_expect(rescue_result.ok and restored.position() == {"route_id":"main", "tile_index":23}, "third EXIT pass returns to the correct main-route point")
	...
	_expect(int(restored.score_breakdown().get("discovery", 0)) >= Session.SCORE_OASIS_EXIT, "rescued exit receives the normal ring-exit reward")
