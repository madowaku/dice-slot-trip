...
func _test_score_and_coin_contract() -> void:
	...
	_expect(pair_started.ok and session.pending_resolution_role() == &"PAIR" and session.score() == 730 and session.skill_gauge() == 1, "third stopped face awards PAIR, bronze mission score, and gauge before movement")
	_consume_hops(session)
	_expect(session.finish_movement().ok, "pre-awarded PAIR movement still settles once")
	_expect(session.score() == 890 and session.coins() == 2 and session.resolution_role() == &"PAIR", "movement, EVENT stop, PAIR, and mission rank produce additive score")
	_expect(session.score_breakdown().slot == Session.SCORE_PAIR, "slot breakdown records the pre-movement role award exactly once")
	_expect(session.acknowledge_resolution() and session.score() == 890, "slot acknowledgment preserves the journey score")
	...
	_expect(straight.resolution_role() == &"STRAIGHT" and straight.score() == 1110 and straight.skill_gauge() == 2, "STRAIGHT is reachable and adds its role, bronze mission award, and two gauge")
	var triple: RefCounted = Session.new()
	_roll_and_finish(triple, 1)
	_roll_and_finish(triple, 1)
	var triple_started: Dictionary = triple.start_roll(1)
	_expect(triple_started.ok and triple.pending_resolution_role() == &"TRIPLE" and triple.skill_gauge() == Session.SKILL_GAUGE_MAX and triple.score() == 1370, "TRIPLE becomes READY and awards bronze mission score without a separate confirmation phase")
...
func _test_boss_victory_score() -> void:
	...
	_expect(session.boss_result().victory, "fourth high roll reaches the 20-space race goal")
	_expect(session.score() >= 3900 and session.best_score() == session.score() and session.score_breakdown().boss == 1500 and session.score_breakdown().finish == 2400, "Sphinx victory, full-HP finish, and no-damage mission preserve additive score contracts")
