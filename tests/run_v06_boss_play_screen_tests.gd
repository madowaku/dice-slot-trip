...
func _test_screen_contract() -> void:
	...
	_expect(is_equal_approx(float(lane_board.call("snapped_camera_for", 10.0)), 10.0), "camera resolves the final player-priority target in one calculation")
	...
	_expect(fixed_camera_settled and is_equal_approx(float(lane_board.get("camera_position")), 0.0) and fixed_camera_elapsed < 400, "camera stays fixed when both raw lane-point token footprints fit the lane viewport")
	...
	_expect(camera_settled and camera_hold_elapsed >= 600 and camera_hold_elapsed < 1000 and is_equal_approx(float(lane_board.get("camera_position")), 10.0), "non-fitting racers produce one player-priority camera target and one Tween")
