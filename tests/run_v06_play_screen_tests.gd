...
func _test_stage_select_source_contract() -> void:
	...
	_expect(source.contains("amazon-city-card.png") and source.contains("kyoto-city-card.png") and source.contains("singapore-city-card.png") and source.contains("newyork-city-card.png") and source.contains("venice-city-card.png"), "stage select imports Amazon, Kyoto, and future destination cards")
	_expect(source.contains("翠雨の大瀑布") and source.contains("千年碁盤の京都") and source.contains("雨あがりの空中庭園シンガポール") and source.contains("眠らない街ニューヨーク") and source.contains("水路迷宮ヴェネツィア"), "stage select names the enabled and future destinations")
	_expect(source.contains("瀑竜アクアフォール") and source.contains("碁盤守：白狐") and source.contains("雨雲のマーライオン") and source.contains("眠らぬリバティキャット") and source.contains("水門の仮面獅子"), "stage select carries each destination's boss label")
	_expect(source.contains("true, _preview_stage.bind(STAGE_KYOTO)") and source.contains("true, _preview_stage.bind(STAGE_AMAZON)") and source.contains("false, _preview_stage.bind(STAGE_SINGAPORE)") and source.contains("false, _preview_stage.bind(STAGE_NEWYORK)") and source.contains("false, _preview_stage.bind(STAGE_VENICE)") and source.contains("locked_cta.disabled = true"), "Amazon and Kyoto are playable while future destinations remain previewable but locked")
	_expect(not source.contains("v0.6 新ルール試遊（保存なし）"), "stage select removes the duplicate legacy trial action")
	_expect(source.contains("_button(\"探検猫で出発\", _start_selected_stage, true)") and source.contains("show_journey_stage(selected_stage_id)"), "stage selection routes enabled destinations to their journey screen")
	_expect(source.contains("func _start_new_v06_game()") and source.contains("GameState.selected_character_id = GameState.DEFAULT_CHARACTER") and source.contains("show_v06_game(selected_stage_id, GameState.DEFAULT_CHARACTER)"), "the Cairo journey still starts V06 with the explorer cat")
