...
func _run() -> void:
	...
	atlas.play_stage_overview_sweep()
	var stage_sweep_receipt: Dictionary = atlas.camera_presentation_receipt()
	_expect(bool(stage_sweep_receipt.active) and str(stage_sweep_receipt.kind) == "stage_sweep" and bool(stage_sweep_receipt.overview), "stage start begins with an animated whole-route sweep")
	atlas.cancel_visual_motion({"route_id":CourseScript.ROUTE_MAIN,"tile_index":0})
	atlas.play_branch_context(CourseScript.ROUTE_BYPASS_BAZAAR)
	var branch_context_receipt: Dictionary = atlas.camera_presentation_receipt()
	_expect(bool(branch_context_receipt.active) and str(branch_context_receipt.kind) == "branch_context" and bool(branch_context_receipt.overview), "shortcut choice temporarily reveals its fork-to-merge context")
	atlas.cancel_visual_motion({"route_id":CourseScript.ROUTE_MAIN,"tile_index":0})
	_expect(atlas.show_branch_comparison(CourseScript.ROUTE_BYPASS_BAZAAR) and atlas.is_overview_mode(), "route choice can keep the fork-to-merge comparison map visible until the player decides")
	atlas.cancel_visual_motion({"route_id":CourseScript.ROUTE_MAIN,"tile_index":0})
	atlas.play_loop_return_context({"route_id":CourseScript.ROUTE_MAIN,"tile_index":24})
	var return_context_receipt: Dictionary = atlas.camera_presentation_receipt()
	_expect(bool(return_context_receipt.active) and str(return_context_receipt.kind) == "loop_return_context" and bool(return_context_receipt.overview), "loop exit temporarily reveals the surrounding main-route location")
	atlas.cancel_visual_motion({"route_id":CourseScript.ROUTE_MAIN,"tile_index":0})
	var presentation_counts: Dictionary = atlas.camera_presentation_receipt().play_counts
	_expect(int(presentation_counts.stage_sweep) == 1 and int(presentation_counts.branch_context) == 1 and int(presentation_counts.loop_return_context) == 1 and atlas.uses_semicircle_carousel(), "all camera previews restore the six-space local route afterward")
