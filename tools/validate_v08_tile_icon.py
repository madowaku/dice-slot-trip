#!/usr/bin/env python3
"""Validate the v0.8 NORMAL footprint icon and provenance contract."""
...
PROVENANCE_PATH = ROOT / "assets/art/v06/tile_kind_icons/normal-footprints.provenance.json"
...
def main() -> int:
    failures: list[str] = []
    check(PROVENANCE_PATH.is_file(), "NORMAL footprint provenance exists", failures)
    if failures:
        return 1

    data = json.loads(PROVENANCE_PATH.read_text(encoding="utf-8"))
    check(data.get("schema_version") == 1, "schema version is supported", failures)
    check(data.get("asset_id") == "normal-footprints", "asset id is canonical", failures)
    ...
    for role, path in paths.items():
        ...
        with Image.open(path) as image:
            check([image.width, image.height] == [record.get("width"), record.get("height")], f"{role} dimensions match provenance", failures)
    ...
    check(bbox == (30, 12, 97, 116), "runtime alpha bounds match footprint optical normalization", failures)
    if bbox is not None:
        check(bbox[3] - bbox[1] == 104 and bbox[2] - bbox[0] < 80, "footprints fill the vertical envelope without becoming a coin-like stack", failures)

    review = data.get("review", {})
    for key in (
        "neutral_walkable_semantics", "exactly_two_footprints", "no_text", "no_logo",
        "no_watermark", "no_arrow", "no_stones", "no_coins", "no_warning_or_risk_symbol",
    ):
    ...
    print(f"V08_TILE_ICON_VALIDATION failures={len(failures)}")
